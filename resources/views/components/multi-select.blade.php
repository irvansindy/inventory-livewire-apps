...
<select class="hidden" x-cloak id="select" wire:model="selected">
	@foreach ($this->allDataInventory as $item)
		<option value="{{ $item->id }}" selected="selected">{{ $item->inventoryName }}</option>
	@endforeach
</select>
			...
			<div x-on:click="open" class="w-full">
				<div class="my-2 p-1 flex border border-gray-200 bg-white rounded">
					<div class="flex flex-auto flex-wrap">
						<template x-for="(option,index) in selected" :key="options[option].value.toString()">
        ...
        <div class="w-full px-4">
			<div x-show.transition.origin.top="isOpen()" class="absolute shadow top-100 bg-white z-40 w-full left-0 rounded max-h-select" x-on:click.away="close">
				<div class="flex flex-col w-full overflow-y-auto">
					<template x-for="(option,index) in options" :key="option.toString()" class="overflow-auto">
